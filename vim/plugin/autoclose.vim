...
" if a CR is typed and we're in between brackets or braces, 
" then we should move move Escape, shift the bracket back (if smartindent
" is being used) one shiftwidth and then start on a newline
function! s:FixBracketPairOnCR()
    let l:result = "\<CR>"
    let l:prev = s:GetPrevChar()
    if s:running && (l:prev == '{' || l:prev == '(')
        let l:result .= "\<Esc>"
        " if has("smartindent")
            " let l:result .= "\<\<"
        " endif
...
exec "inoremap <silent> <CR> <C-R>=<SID>SetVEAll()<CR><C-R>=<SID>FixBracketPairOnCR()<CR><C-R>=<SID>RestoreVE()<CR>"
